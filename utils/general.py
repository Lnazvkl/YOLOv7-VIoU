...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, VIOU=False, eps=1e-7):
    ...
    if GIoU or DIoU or CIoU or VIOU:
        cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)  # convex (smallest enclosing box) width
        ch = torch.max(b1_y2, b2_y2) - torch.min(b1_y1, b2_y1)  # convex height
        if CIoU or DIoU or VIOU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if DIoU:
                ...
                v = (8 / (3 * math.pi ** 2)) * (torch.pow(torch.atan(w2 / (w1 + eps)) - (math.pi / 4), 2) +
                                          torch.pow(torch.atan(h2 / (h1 + eps)) - (math.pi / 4), 2) +
                                                torch.pow(torch.atan((w2 * h2 - w1 * h1) / (w2 * h2 + eps)), 2))
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
...
def bbox_alpha_iou(box1, box2, x1y1x2y2=False, GIoU=False, DIoU=False, CIoU=False, VIOU=False, alpha=2, eps=1e-9):
